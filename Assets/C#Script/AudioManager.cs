...
public class AudioManager : MonoBehaviour
{
    ...
    private AudioSource sfx2DSource; //2D music for sfx
    private AudioSource[] musicSources; //array for BGM
    private int activeMusicSourceIndex; //current active BGM

    public static AudioManager instance; //to let other script access to audio manager

    private Transform audioListener; //Listener position
    private Transform playerT; //player position

    private SoundLibrary library; //reference to SoundLibrary script
    ...
    void Awake()
    {
        ...
        else
        {
            ...
            if(FindObjectOfType<FPSInput>() != null) //if there is a player
            {
                playerT = FindObjectOfType<FPSInput>().transform; //find position of player
            }
    ...
    void OnLevelWasLoaded(int index)
    {
        if (playerT == null)
        {
            if (FindObjectOfType<FPSInput>() != null)
            {
                playerT = FindObjectOfType<FPSInput>().transform;
            }
        }
    }
